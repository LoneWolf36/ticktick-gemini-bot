...
title: Env Var Standardization and Context Template Update
dependencies:
- WP02
- WP03
requirement_refs:
...
# Work Package Prompt: WP04 - Env Var Standardization and Context Template Update
...
Make local setup and user-context configuration accurate enough that the behavioral assistant can be run without guesswork.

**Independent test**: A maintainer can compare `server.js`, `.env.example`, README, Render config, and `services/user_context.example.js` without finding contradictory required variables or stale context shapes.

Success looks like:
- Required configuration is consistent across files.
- User context template includes goal themes and behavioral hints only at the level current code can consume.
- No secret or personal data is introduced.
...
- Implementation command: `spec-kitty implement WP04 --mission 004-post-migration-cleanup`
...
### Subtask T014 - Align environment variable references

**Purpose**: Remove setup contradictions.

**Required work**:
- Compare `server.js`, `.env.example`, README, and `render.yaml`.
- Add missing documentation for runtime-used variables.
- Remove or annotate stale docs for unused variables.

**Acceptance checks**:
- Every runtime-required variable is documented.
- Optional variables are not mislabeled required.
- Docs agree across files.

### Subtask T015 - Update user context template

**Purpose**: Make behavioral inputs understandable without overbuilding memory.

**Required work**:
- Review `services/user_context.example.js`.
- Add comments for goal themes, prioritization hints, and behavioral pattern placeholders only where current or accepted specs need them.
- Avoid storing sensitive or raw behavioral history by default.

**Acceptance checks**:
- Template matches current consumers.
- Future behavioral sections are labeled clearly.
- No personal real data is committed.

### Subtask T016 - Verify runtime validation wording

**Purpose**: Ensure docs match startup behavior.

**Required work**:
- Inspect `REQUIRED_VARS` and startup validation in `server.js`.
- Update docs where required/optional mismatch exists.
- Do not change runtime validation unless docs prove it is wrong and tests cover it.

**Acceptance checks**:
- Startup validation and docs align.
- Changes are covered by regression or syntax checks if runtime changes.
- No unrelated startup behavior changes.

## Risks and Mitigations

- Risk: context template implies memory features that are not implemented. Mitigation: mark planned/future sections explicitly.
- Risk: required env var list becomes stricter than runtime. Mitigation: compare to `REQUIRED_VARS`.
