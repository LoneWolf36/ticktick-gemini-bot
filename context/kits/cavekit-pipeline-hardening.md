...
- [x] R12 completed: `pipeline.js` defers parsed intents on API unavailability with user messaging, `store.js` provides `deferredPipelineIntents` CRUD (`appendDeferredPipelineIntent`, `getDeferredPipelineIntents`, `removeDeferredPipelineIntent`) for persistence and retry.
