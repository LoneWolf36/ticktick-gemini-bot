...
function normalizeScheduleMetadata(
  rawContext = {},
  entryPoint = "manual_command",
) {
  const schedulingMetadata =
    rawContext.schedulingMetadata || rawContext.scheduling_metadata || {};
  const triggerKind =
    schedulingMetadata.triggerKind || schedulingMetadata.trigger_kind;

  return {
    triggerKind:
      triggerKind === "scheduled" || entryPoint === "scheduler"
        ? "scheduled"
        : "manual",
    scheduleKey:
      schedulingMetadata.scheduleKey || schedulingMetadata.schedule_key || null,
    scheduledForIso:
      schedulingMetadata.scheduledForIso ||
      schedulingMetadata.scheduled_for_iso ||
      null,
...
function normalizeSummaryRequestContext(kind, rawContext = {}) {
  ...
  const deliveryChannel =
    rawContext.deliveryChannel || rawContext.delivery_channel;
  const normalizedEntryPoint = ENTRY_POINT_VALUES.has(entryPoint)
    ? entryPoint
    : "manual_command";
  ...
  return {
    ...
    generatedAtIso:
      rawContext.generatedAtIso ||
      rawContext.generated_at_iso ||
      new Date().toISOString(),
    timezone: rawContext.timezone || null,
    workStyleMode: WORK_STYLE_MODE_VALUES.has(workStyleMode)
      ? workStyleMode
      : "standard",
    urgentMode:
      rawContext.urgentMode === true || rawContext.urgent_mode === true,
    deliveryChannel: DELIVERY_CHANNEL_VALUES.has(deliveryChannel)
      ? deliveryChannel
      : "telegram",
    schedulingMetadata: normalizeScheduleMetadata(
      rawContext,
      normalizedEntryPoint,
    ),
    tonePolicy: TONE_POLICY_VALUES.has(tonePolicy)
      ? tonePolicy
      : "preserve_existing",
    ticktickFetchFailed: rawContext.ticktickFetchFailed === true,
    excludedTaskIds: Array.isArray(rawContext.excludedTaskIds)
      ? rawContext.excludedTaskIds
      : [],
  };
}

function normalizeSummaryNotice(notice = {}) {
  const code = SUMMARY_NOTICE_CODES.includes(notice.code)
    ? notice.code
    : "delivery_context";
  const severity = SUMMARY_NOTICE_SEVERITIES.includes(notice.severity)
    ? notice.severity
    : "info";
  const evidenceSource = SUMMARY_NOTICE_EVIDENCE_SOURCES.includes(
...
export function normalizeWeeklyWatchouts(watchouts = []) {
  return toArray(watchouts)
    ...
    .map((watchout) => {
      ...
      const evidenceSource = WEEKLY_WATCHOUT_EVIDENCE_SOURCES.includes(
...
export function normalizeWeeklySummary(summary = {}) {
  return {
    progress: toArray(summary.progress)
      .map((item) => toString(item))
      .filter(Boolean),
    ...
    next_focus: toArray(summary.next_focus)
      .map((item) => toString(item))
      .filter(Boolean),
...
export function createSummaryDiagnostics({
  ...
  const normalizedContext = normalizeSummaryRequestContext(
    context.kind,
    context,
  );
  const tonePreserved = formattedResult?.tonePreserved ?? true;
  const telegramSafe =
    typeof formattedResult?.telegramSafe === "boolean"
  ...
  return {
    ...
    formatterVersion:
      formattedResult?.formatterVersion || SUMMARY_FORMATTER_VERSION,
    formattingDecisions: {
      telegramSafe,
      tonePreserved,
...
export function composeBriefingSummary({
  ...
  const formattedResult = formatSummary({
    kind: BRIEFING_KIND,
    summary,
    context: normalizedContext,
  });
