...
export class GeminiAnalyzer {
    ...
    async _prepareBriefingTasks(tasks = [], options = {}) {
        ...
        const orderedTasks = ranking.ranked.map((decision) => byTaskId.get(decision.taskId)).filter(Boolean);
    ...
    _isDailyQuotaError(err) {
        ...
        const retryMatch =
            (err?.message || '').match(/retry\s+(?:after|in)\s+([\d\.]+)\s*(s|sec|seconds|m|min|minutes|h|hours)?/i) ||
    ...
    async _executeWithFailover(
        prompt,
        apiCallFn,
        { transientBaseMs = 15, modelTier = 'fast', interactiveWritePath = false } = {}
    ) {
        // Global pre-checks
        ...
        for (let modelIndex = 0; modelIndex < chain.length; modelIndex++) {
            ...
            while (true) {
                ...
                const maskedKey =
                    this._keys[currentKeyIndex].slice(0, 4) + '...' + this._keys[currentKeyIndex].slice(-4);
        ...
        if (hasQuotaExhaustion) {
            throw new AIHardQuotaError('All model fallback keys exhausted due to quota limits.', {
                status: 429,
                scope: 'account'
            });
        }
        if (hasInvalidKey) {
            throw new AIInvalidKeyError('All model fallback keys exhausted due to invalid keys.', {
                status: 403,
                scope: 'account'
            });
        }
        if (hasServiceUnavailable) {
            throw new AIServiceUnavailableError('All model fallback keys exhausted due to service unavailability.', {
                status: 503,
                scope: 'account'
            });
        }
    ...
    async generateWeeklyDigestSummary(allTasks, processedThisWeek, options = {}) {
        ...
        const processedEntries = Object.entries(processedThisWeek || {}).map(([taskId, data]) => ({ taskId, ...data }));
        const processed = processedEntries.map((entry) => formatProcessedTask(entry)).join('\n');
        const behavioralPatterns = await this._resolveBehavioralPatterns(options);
