...
import {
  loadUserContextModule,
  getModuleExport,
} from "./user-context-loader.js";
import {
  userTodayFormatted,
  PRIORITY_EMOJI,
  formatProcessedTask,
...
} from "./summary-surfaces/index.js";
import {
...
if (userContext) {
  ...
  USER_CONTEXT =
    "You are an AI accountability partner and task analyst. Help the user stay focused, organized, and honest about their priorities.";
  USER_CONTEXT_SOURCE = "fallback";
  console.warn(
    "⚠️  No user_context.js found and USER_CONTEXT env var not set. Using generic context.",
  );
}
...
export class AIServiceUnavailableError extends Error {
  constructor(
    message,
    { status, model, keyIndex, retryAfterMs, resumeAt, scope } = {},
  ) {
    super(message);
...
--------------------------------
1. Follow schema exactly (no missing or extra fields)
2. Use only evidence from provided task data
3. Keep output concise within limits
4. Apply tone/style rules
...
- Avoid behavioral callouts
- Focus on task evidence and ranking rationale
- If data is sparse, return compact output
...

{
  ...
  "priorities": [
    {
      ...
      "priority_label": "low"|"medium"|"high"|"none",
  ...
  "notices": [
    {
      "code": "sparse_tasks"|"degraded_ranking"|"urgent_mode_active"|"delivery_context",
      "message": string,
      "severity": "info"|"warning",
      "evidence_source": "tasks"|"processed_history"|"state"|"system"
    }
  ]
}
...
- why_now: max 3 items, each ≤ 80 chars
- notices: max 3 items
...
2. Ground all outputs in provided data
3. Keep output concise within limits
4. Apply tone/style rules
...

{
  ...
  "next_focus": string[],
  "watchouts": [
    { "label": string, "evidence": string, "evidence_source": "current_tasks"|"processed_history" }
  ],
  "notices": [
    { "code": string, "message": string, "severity": string, "evidence_source": string }
  ]
}
...
CONSTRAINTS
--------------------------------
- progress: max 5 items, each ≤ 100 chars
- next_focus: EXACTLY 3 items, each ≤ 80 chars
- watchouts: max 3 items
- carry_forward: max 5 items
...
export class GeminiAnalyzer {
  ...
  constructor(apiKeys, config = {}) {
    ...
    if (this._keys.length === 0) {
      throw new Error(
        "❌ Gemini API key not set! Get one from https://aistudio.google.com/apikey",
      );
    }
    ...
    this._modelTiers = {
      fast: this._buildTierChain(
        ...
        config.modelFastFallbacks?.length
      ...
      advanced: this._buildTierChain(
        ...
        config.modelAdvancedFallbacks?.length
          ? config.modelAdvancedFallbacks
          : [
              "gemini-2.5-flash",
  ...
  _prepareBriefingTasks(tasks = [], options = {}) {
    ...
    const goalThemeProfile =
      options.goalThemeProfile ||
      createGoalThemeProfile(USER_CONTEXT, { source: USER_CONTEXT_SOURCE });
    ...
    const byTaskId = new Map(
      activeTasks.map((task) => [task.id || task.taskId, task]),
    );
  ...
  async _resolveBehavioralPatterns(options = {}) {
    ...
    try {
      ...
      return this._detectBehavioralPatternsForSummary(signals, {
        nowMs:
          Date.parse(options.generatedAtIso || new Date().toISOString()) ||
          Date.now(),
      });
    } catch {
      return [];
  ...
  _isDailyQuotaError(err) {
    ...
    const retryMatch =
      (err?.message || "").match(
        /retry\s+(?:after|in)\s+([\d\.]+)\s*(s|sec|seconds|m|min|minutes|h|hours)?/i,
  ...
  _getModelKeyState(model, keyIndex) {
    if (!this._modelKeyExhaustion.has(model)) {
      this._modelKeyExhaustion.set(
        model,
        new Array(this._keys.length)
          .fill(null)
          .map(() => ({ until: null, reason: null })),
      );
    }
  ...
  _isCircuitOpen(model) {
    ...
    if (state.openUntil && Date.now() < state.openUntil) {
      console.warn(
        `[CircuitBreaker] ${JSON.stringify({ eventType: "ai.circuit_breaker.blocked", model, openUntil: state.openUntil })}`,
      );
      return true;
    }
    if (state.openUntil && Date.now() >= state.openUntil) {
  ...
  _recordCircuitFailure(model, kind) {
    ...
    const recent60s503 = state.failures.filter(
      (f) => f.kind === "503" && now - f.at <= 60 * 1000,
    );
  ...
  _recordCircuitSuccess(model) {
    ...
    if (wasHalfOpen) {
      console.log(
        `[CircuitBreaker] ${JSON.stringify({ eventType: "ai.circuit_breaker.close", model })}`,
      );
    }
  }
  ...
  _getFailureCounts(model) {
    if (!this._aiFailureCounts.has(model)) {
      this._aiFailureCounts.set(model, {
        _503: 0,
        _429_quota: 0,
        _429_rate: 0,
        invalid_key: 0,
  ...
  _recordAiFailure(model, kind, keyIndex) {
    ...
    counts.total++;
    console.warn(
      `[AIFailure] ${JSON.stringify({ eventType: "ai.failure", model, kind, keyIndex })}`,
    );
    if (counts.total % 10 === 0) {
      console.warn(
        `[AIFailure] ${JSON.stringify({
  ...
  quotaResumeTime() {
    ...
    const nonNulls = this._exhaustedUntilByKey.filter(
      (t, idx) =>
        t !== null && this._keyUnavailableReason[idx] === "daily_quota",
    );
  ...
  isQuotaExhausted() {
    ...
    for (let i = 0; i < this._keys.length; i++) {
      ...
      if (this._keyUnavailableReason[i] !== "daily_quota") {
        allKeysQuotaExhausted = false;
        break;
      }
    }
   ...
   * @private
   */
  async _executeWithFailover(
    prompt,
    apiCallFn,
    {
      transientBaseMs = 15,
      modelTier = "fast",
      interactiveWritePath = false,
    } = {},
  ) {
    // Global pre-checks
    if (this._areAllKeysUnavailable()) {
      throw new AIInvalidKeyError("All API keys are unavailable.", {
        status: 403,
        scope: "account",
      });
    }
    ...
    for (let modelIndex = 0; modelIndex < chain.length; modelIndex++) {
      ...
      if (modelIndex > 0) {
        console.log(
          `⚡ [Gemini] Model fallback: ${chain[modelIndex - 1]} exhausted → trying ${model} (tier: ${modelTier})`,
        );
      }
      ...
      if (this._isCircuitOpen(model)) {
        console.log(
          `⏭️ [Gemini] Circuit breaker open for ${model}, skipping to next model in chain`,
        );
        continue;
      }
      ...
      if (this._areAllKeysExhaustedForModel(model)) {
        console.log(
          `⏭️ [Gemini] All keys exhausted for ${model}, skipping to next model in chain`,
        );
        continue;
      }
      ...
      while (true) {
        if (!this._isModelKeyAvailable(model, currentKeyIndex)) {
          const nextIdx = this._findNextAvailableKeyForModel(
            model,
            currentKeyIndex,
          );
        ...
        const maskedKey =
          this._keys[currentKeyIndex].slice(0, 4) +
          "..." +
          this._keys[currentKeyIndex].slice(-4);
        ...
        try {
          ...
          if (usage) {
            console.log(
              `📊 [Gemini API] Tokens -> In: ${usage.promptTokenCount} | Out: ${usage.candidatesTokenCount} | Total: ${usage.totalTokenCount}`,
            );
          }

          if (modelIndex > 0) {
            console.log(
              `✅ [Gemini] Fallback model ${model} succeeded (tier: ${modelTier})`,
            );
          }
          ...
          const maskedKey = activeK
            ? `${activeK.slice(0, 4)}...${activeK.slice(-4)}`
            : "undefined";
          console.error(
            `[DIAGNOSTICS] Key ${currentKeyIndex + 1}/${this._keys.length} [${maskedKey}] exactly threw: [${err.status}] ${err.message}`,
          );
          ...
          if (this._isInvalidApiKeyError(err)) {
            ...
            console.error(
              `🚫 Key ${currentKeyIndex + 1}/${this._keys.length} marked unavailable (invalid/leaked).`,
            );
            const nextIdx = this._findNextAvailableKeyForModel(
              model,
              currentKeyIndex,
          ...
          if (this._isDailyQuotaError(err)) {
            ...
            if (this._isGlobalQuotaError(err)) {
              this._markActiveKeyUnavailable(
                "daily_quota",
                Date.now() + resetMs,
              );
            }
            ...
            if (nextIdx !== -1) {
              ...
              if (rotations <= maxRotations) {
                console.log(
                  `🔄 Rotating to next key for model ${model} (daily quota)`,
                );
                continue;
              }
            }
            ...
            if (this.isQuotaExhausted()) {
              ...
              const resumeStr = resumeTime
                ...
                : "unknown";
              console.error(
                `🛑 All AI keys exhausted — pausing calls until ~${resumeStr} PT.`,
              );
            } else {
              console.error(
                `⚠️ Daily quota hit for ${model} across ${rotations + 1} attempted keys. Moving to next model in chain.`,
              );
            }
          ...
          if (isTransient) {
            ...
            if (isServiceUnavailable && interactiveWritePath) {
              ...
              console.error(
                `⏳ Service unavailable for ${model}, fast-falling back to next model (write path)`,
              );
              break;
            }

            const effectiveMaxRetries = interactiveWritePath
              ? 1
              : maxTransientRetries;
            if (transientAttempts < effectiveMaxRetries) {
              ...
              const errorType = isServiceUnavailable
                ...
                : "rate limited";
              console.error(
                `⏳ ${errorType}, waiting ${Math.round(backoffMs / 1000)}s before retry ${transientAttempts + 1}/${effectiveMaxRetries}...`,
              );
            ...
            const errorType = isServiceUnavailable
              ...
              : "Rate limit";
            console.error(
              `⏳ ${errorType} retries exhausted for ${model}, trying next model in chain.`,
            );
            if (isServiceUnavailable) {
              this._recordCircuitFailure(model, "503");
            }
      ...
      if (modelHitQuota && this._areAllKeysExhaustedForModel(model)) {
        ...
        console.log(
          `[Gemini] Model ${model} marked permanently exhausted (all keys quota-depleted)`,
        );
      }
    }
    ...
    if (hasQuotaExhaustion) {
      throw new AIHardQuotaError(
        "All model fallback keys exhausted due to quota limits.",
        { status: 429, scope: "account" },
      );
    }
    if (hasInvalidKey) {
      throw new AIInvalidKeyError(
        "All model fallback keys exhausted due to invalid keys.",
        { status: 403, scope: "account" },
      );
    }
    if (hasServiceUnavailable) {
      throw new AIServiceUnavailableError(
        "All model fallback keys exhausted due to service unavailability.",
        { status: 503, scope: "account" },
      );
    }
    throw new AIServiceUnavailableError("All model fallback keys exhausted.", {
      status: 503,
      scope: "account",
    });
  }
  ...
  async generateDailyBriefingModelSummary(tasks, options = {}) {
    ...
    const rankedPreview = ranking.ranked
      ...
      .map((decision, index) => {
        const task = orderedTasks.find(
          (candidate) => (candidate.id || candidate.taskId) === decision.taskId,
        );
    ...
    const workStylePromptNote = buildWorkStylePromptNote(
      recommendationState.workStyleMode,
    );
  ...
  async generateDailyBriefingSummary(tasks, options = {}) {
    const { modelSummary, ranking, orderedTasks, recommendationState } =
  ...
  async generateWeeklyDigestSummary(allTasks, processedThisWeek, options = {}) {
    ...
    const processedEntries = Object.entries(processedThisWeek || {}).map(
    ...
    return composeWeeklySummary({
      context: {
        ...
        urgentMode: recommendationState.urgentMode,
        ticktickFetchFailed: options.ticktickFetchFailed === true,
        excludedTaskIds: Array.isArray(options.excludedTaskIds)
  ...
  async generateDailyCloseSummary(allTasks, processedTasks, options = {}) {
    ...
    const processedEntries = Array.isArray(processedTasks)
      ...
      : Object.entries(processedTasks || {}).map(([taskId, data]) => ({
          taskId,
          ...data,
        }));
