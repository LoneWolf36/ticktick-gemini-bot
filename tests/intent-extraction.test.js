...
describe('Intent Extraction', () => {
    ...
    describe('createIntentExtractor', () => {
        it('throws when gemini is not a GeminiAnalyzer instance', () => {
            assert.throws(() => createIntentExtractor({}), /createIntentExtractor requires a GeminiAnalyzer instance/);
        });
...
describe('validateChecklistItems', () => {
    ...
    it('rejects items with invalid status', () => {
        const result = validateChecklistItems([{ title: 'Good item' }, { title: 'Bad status', status: 'done' }]);
        assert.equal(result.valid, true);
        assert.equal(result.items.length, 1);
    ...
    it('rejects non-object items in array', () => {
        const result = validateChecklistItems([{ title: 'Valid' }, 'not an object', 42]);
        assert.equal(result.valid, true);
        assert.equal(result.items.length, 1);
    ...
    it('trims whitespace from titles', () => {
        const result = validateChecklistItems([{ title: '  Trimmed title  ' }]);
        assert.equal(result.valid, true);
