...
test('WP04 T046: checklist create creates one parent task with items', async () => {
    ...
    const result = await processMessage(
        'Onboard new client: send welcome email, create project folder, schedule kickoff'
    );

    assert.equal(result.type, 'task');
    assert.equal(adapterCalls.create.length, 1);
    assert.equal(adapterCalls.create[0].checklistItems.length, 3);
});
...
test('R5: checklist create confirmation stays terse and includes item count', async () => {
        ...
        intents: [
            {
                ...
                confidence: 0.95,
                projectHint: 'Career',
                checklistItems: [{ title: 'Book flights' }, { title: 'Pack bags' }, { title: 'Renew travel card' }]
            }
        ]
    });
    ...
    assert.match(standard.confirmationText, /\(3 items\)/);
    assert.match(urgent.confirmationText, /\(3 items\)/);
...
test('WP04 T046: multi-task create creates separate tasks without checklist', async () => {
    ...
    for (const created of adapterCalls.create) {
        assert.equal(
            Object.hasOwn(created, 'checklistItems') &&
                Array.isArray(created.checklistItems) &&
...
test('WP05 P0#1: pipeline resolves ambiguity with checklistPreference=separate', async () => {
    ...
    assert.equal(result.type, 'task');
    assert.equal(adapterCalls.create.length, 2);
});
...
test('WP05 P0#2: cl:checklist callback resumes pipeline with checklistPreference', async () => {
    ...
    assert.ok(
        replies.some((r) => r?.text && r.text.includes('Created task with checklist')),
        'should show pipeline result'
    );
    assert.ok(
        replies.some((r) => r?.opts?.reply_markup),
        'should include undo when persisted'
    );
...
test('WP05 P0#2: cl:separate callback resumes pipeline with separate preference', async () => {
    ...
    assert.ok(
        replies.some((r) => r && r.includes('Created separate tasks')),
        'should show pipeline result'
    );
});
...
test('WP05 P0#2: cl:skip callback resumes pipeline with skipChecklist=true', async () => {
    ...
    assert.ok(
        replies.some((r) => r && r.includes('Created single task')),
        'should show pipeline result'
    );
});
...
test('WP05 P0#2: checklist callback rejects unauthorized user', async () => {
    ...
    await registerCallbacksForTest(
        bot,
        { isAuthenticated: () => true },
        {
...
test('WP05 P0#2: checklist callback rejects cross-chat user', async () => {
    ...
    assert.equal(
        replies.some((r) => r && r.includes('Wrong chat')),
        true,
        'should reject cross-chat'
    );
});
