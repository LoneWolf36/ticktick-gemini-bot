...
test('pipeline context resolves dentist Thursday through the normalizer path', async () => {
    ...
    assert.deepEqual(
        axCalls[0].options.availableProjects,
...
test('createIntentExtractor propagates QuotaExhaustedError when all models exhausted', async () => {
    ...
    await assert.rejects(
        () =>
            intentExtractor.extractIntents('schedule rent', {
                currentDate: '2026-03-10',
                availableProjects: ['Inbox'],
                requestId: 'req-intent-quota'
            }),
        QuotaExhaustedError
    );
...
test('burst pipeline requests remain isolated and deterministic', async () => {
    ...
    const results = await Promise.all(
        requests.map(({ message, requestId }) =>
...
test('pipeline happy path covers create, update, complete, delete, and non-task routing', async () => {
    ...
    const createHarness = createPipelineHarness({
        intents: [{ type: 'create', title: 'Write summary', confidence: 0.9, projectHint: 'Career' }]
    });
    ...
    const completeHarness = createPipelineHarness({
        intents: [{ type: 'complete', taskId: 'task-456', projectHint: 'Inbox', confidence: 0.9 }]
    });
    ...
    const deleteHarness = createPipelineHarness({
        intents: [{ type: 'delete', taskId: 'task-789', projectHint: 'Inbox', confidence: 0.9 }]
    });
...
test('registerCallbacks wires mut:pick and mut:cancel callback families', async () => {
    ...
    registerCallbacks(bot, {}, { processMessage: async () => ({ type: 'non-task', confirmationText: 'Got it' }) });
...
test('mut:pick resumes through pipeline with resolved task context', async () => {
    ...
    const pipeline = {
        processMessage: async (msg, opts) => {
            ...
            return {
                type: 'task',
                confirmationText: 'Updated "Write weekly report"',
                actions: [],
...
test('mut:confirm resumes through pipeline with skipMutationConfirmation', async () => {
    ...
    const pipeline = {
        processMessage: async (msg, opts) => {
            ...
            return {
                type: 'task',
                confirmationText: 'Deleted 1 task',
                actions: [],
                results: [{ status: 'succeeded' }]
            };
        }
...
test('mut:confirm blocks expired pending confirmations fail-closed', async () => {
    ...
    registerCallbacks(
        ...
        {
            listActiveTasks: async () => {
                throw new Error('should not list tasks');
            }
        },
        {
            processMessage: async () => {
...
test('WP07 T071: update mutation happy path — rename target via exact match', async () => {
    const harness = createPipelineHarness({
        intents: [{ type: 'update', title: 'Weekly report — Q4', confidence: 0.95, targetQuery: 'weekly' }],
        activeTasks: [
            {
                id: 'task-update-01',
                title: 'Weekly report draft',
...
test('WP07 T071: complete mutation happy path — exact match', async () => {
    const harness = createPipelineHarness({
        intents: [{ type: 'complete', title: 'Review PR #123', confidence: 0.95, targetQuery: 'review PR' }],
        activeTasks: [
            {
                id: 'task-complete-01',
                title: 'Review PR #123',
...
test('WP07 T071: delete mutation happy path — exact match', async () => {
    const harness = createPipelineHarness({
        intents: [{ type: 'delete', title: 'Buy groceries', confidence: 0.95, targetQuery: 'groceries' }],
        activeTasks: [
            {
                id: 'task-delete-01',
                title: 'Buy groceries',
...
test('WP07 T072: batch-style mutation phrasing is rejected with single-target guidance', async () => {
    const harness = createPipelineHarness({
        intents: [{ type: 'update', title: null, targetQuery: 'all gym tasks', dueDate: 'next week', confidence: 0.9 }],
        activeTasks: [
...
test('WP07 T072: ambiguous matches require clarification instead of mutation', async () => {
    const harness = createPipelineHarness({
        intents: [{ type: 'update', title: 'report', confidence: 0.9, targetQuery: 'report' }],
        activeTasks: [
            {
                id: 'task-amb-01',
                title: 'Weekly report',
...
test('WP07 T072: not-found result does not mutate', async () => {
    const harness = createPipelineHarness({
        intents: [{ type: 'update', title: 'nonexistent task', confidence: 0.9, targetQuery: 'nonexistent task xyz' }],
        activeTasks: [
...
test('WP07 T072: delete remains fail-closed when resolution is uncertain', async () => {
    const harness = createPipelineHarness({
        intents: [{ type: 'delete', title: 'task', confidence: 0.4, targetQuery: 'task' }],
        activeTasks: [
            {
                id: 'task-fc-01',
