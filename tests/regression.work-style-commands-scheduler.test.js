...
test('focus mode suppresses non-critical scheduler notifications but allows critical alerts', () => {
    assert.equal(
        shouldSuppressScheduledNotification(store.MODE_FOCUS, SCHEDULER_NOTIFICATION_TYPES.DAILY_BRIEFING),
        true
    );
    assert.equal(
        shouldSuppressScheduledNotification(store.MODE_FOCUS, SCHEDULER_NOTIFICATION_TYPES.WEEKLY_DIGEST),
        true
    );
    assert.equal(
        shouldSuppressScheduledNotification(store.MODE_FOCUS, SCHEDULER_NOTIFICATION_TYPES.PENDING_SUPPRESSION),
        true
    );
    assert.equal(shouldSuppressScheduledNotification(store.MODE_FOCUS, SCHEDULER_NOTIFICATION_TYPES.AUTO_APPLY), true);
    assert.equal(
        shouldSuppressScheduledNotification(store.MODE_FOCUS, SCHEDULER_NOTIFICATION_TYPES.TOKEN_EXPIRED),
        false
    );
    assert.equal(
        shouldSuppressScheduledNotification(store.MODE_FOCUS, SCHEDULER_NOTIFICATION_TYPES.QUOTA_EXHAUSTED),
        false
    );
    assert.equal(
        shouldSuppressScheduledNotification(store.MODE_STANDARD, SCHEDULER_NOTIFICATION_TYPES.DAILY_BRIEFING),
        false
    );
});
...
test('registerCommands wires /urgent to the work-style store contract', async () => {
    const authChatId = await (async () => {
        const m = await import('../services/shared-utils.js');
        return m.AUTHORIZED_CHAT_ID;
    })();
...
test('registerCommands allows free-form work-style changes before TickTick auth', async () => {
    ...
    assert.equal(
        replies.some((message) => /TickTick not connected yet/i.test(message)),
        false
    );
});
...
test('registerCommands uses shared briefing surface and preserves urgent reminder', async () => {
    ...
    assert.ok(
        replies.some((message) => typeof message === 'string' && message.includes('Urgent mode is currently active.'))
    );
});
...
test('registerCommands uses shared weekly surface and sends formatted output', async () => {
    ...
    assert.ok(
        replies.some((message) => typeof message === 'string' && message.includes('WEEKLY ACCOUNTABILITY REVIEW'))
    );
});
