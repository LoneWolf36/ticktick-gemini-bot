...
test('TickTickAdapter createTask omits items when checklistItems is null or undefined', async () => {
    ...
    assert.equal(
        Object.hasOwn(createPayload, 'items'),
        false,
        'items should NOT be present when checklistItems is undefined'
    );
...
test('pipeline retries once and rolls back earlier successful writes', async () => {
    ...
    assert.deepEqual(
        telemetryEvents.filter((event) => event.eventType === 'pipeline.execute.failed').map((event) => event.attempt),
        [1]
    );
    assert.ok(
...
test('GeminiAnalyzer classifies invalid API key errors and repairs sloppy JSON', () => {
    ...
    const repaired = analyzer._safeParseJson(
        "{summary:'ok',actions:[{type:'update',taskId:'1',changes:{priority:3,}},],}"
    );
...
test('GeminiAnalyzer rotates to next key on invalid-key errors', async () => {
    ...
    const result = await analyzer._executeWithFailover('noop prompt', async (ai, prompt, model) => {
        callCount++;
        if (callCount === 1) {
...
test('GeminiAnalyzer builds work-style prompt notes for standard, focus, and urgent modes', () => {
    ...
    assert.match(
        buildWorkStylePromptNote(store.MODE_STANDARD),
        /Do not imply urgency unless the user explicitly activated urgent mode/i
    );
    assert.match(
        buildWorkStylePromptNote(store.MODE_STANDARD),
        /When confidence is low, label uncertainty, ask, or stay quiet/i
    );
    assert.match(buildWorkStylePromptNote(store.MODE_STANDARD), /Use silent signals first/i);
    assert.match(
        buildWorkStylePromptNote(store.MODE_STANDARD),
        /Direct call-outs only when repeated evidence justifies them/i
    );
    ...
    assert.match(
        buildWorkStylePromptNote(store.MODE_FOCUS),
        /Do not imply urgency unless the user explicitly activated urgent mode/i
    );
    assert.match(
        buildWorkStylePromptNote(store.MODE_FOCUS),
    ...
    assert.match(
        buildWorkStylePromptNote(store.MODE_URGENT),
        /direct, assertive, action-oriented language only when task evidence, deadlines, or explicit user context justify it/i
    );
    assert.match(buildWorkStylePromptNote(store.MODE_URGENT), /Be non-judgmental: no shame, blame, or moralizing/i);
    assert.match(
        buildWorkStylePromptNote(store.MODE_URGENT),
        /If a task mutation is ambiguous, ask for clarification instead of guessing/i
    );
    assert.match(
        buildWorkStylePromptNote(store.MODE_URGENT),
        /Reflect urgency only because the user explicitly activated urgent mode; never invent urgency/i
    );
    assert.match(
        buildWorkStylePromptNote(store.MODE_URGENT),
        /When confidence is low, label uncertainty, ask, or stay quiet/i
    ...
    assert.match(
        buildWorkStylePromptNote(store.MODE_URGENT),
        /Urgent mode does not lower the confidence threshold for behavioral claims/i
    );
    ...
    assert.match(
        buildWorkStylePromptNote(store.MODE_URGENT),
        /Strict commands are allowed only because urgent mode was explicitly activated/i
    );
    ...
    assert.match(
        buildWorkStylePromptNote(store.MODE_URGENT),
        /Strip only formatting niceties; preserve substantive content/i
    );
    assert.match(
        buildWorkStylePromptNote(store.MODE_URGENT),
        /do not mutate TickTick state unless the user explicitly asks/i
    );
...
test('urgent mode trims niceties without skipping safety or substantive content', async () => {
    ...
    const weeklySummary = {
        ...
        watchouts: [{ label: 'Overdue tasks accumulating', evidence: '4 tasks slipped this week' }],
...
test('composeWeeklySummary stays silent on a single ignored suggestion', () => {
    ...
    assert.equal(
        result.summary.notices.some((notice) => notice.code === 'engagement_pattern'),
        false
    );
});
